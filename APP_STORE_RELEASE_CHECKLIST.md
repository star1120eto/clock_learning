...
### B-4. 🟡 入力完了・Apple側の処理待ち — 有料 App 契約（Paid Applications Agreement）
...
| 無料アプリ契約 | ✅ 有効 |
| 有料アプリ契約 | 🟡 処理中 |
| 銀行口座（ゆうちょ銀行） | 🟡 処理中（更新内容は24時間後に反映、と画面表示あり） |
| U.S. Form W-8BEN | ✅ 有効（第12条・0%・Income from the sale of applications で申請） |
| U.S. Certificate of Foreign Status | ✅ 有効（Title: Owner で送信） |
| デジタルサービス法（EU DSA） | ✅ 有効 |

**入力作業はすべて完了。** 残るは銀行口座反映後、有料アプリ契約が自動で「有効」に
変わるのを待つのみ（操作不要）。24時間経っても「処理中」のままの場合や
エラー表示に変わった場合は、ゆうちょ銀行特有の入力（通帳の記号・番号ではなく、
他行振込用の店名〈3桁〉・預金種目・口座番号〈7桁〉）に誤りがないか確認する。
...
iPad 対応は公開後のアップデートに回したため（A-6）、iPad 用スクリーンショットは不要。
